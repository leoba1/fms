...
function partialRefresh() {
    $("#fileList ul").html(`<li class="fl-li" id="addFile"><img src="./assets/bigFile/img/添加.png"><label>添加文件</label></li>`);
    $.ajax({
        type: "get",
            ...
            if (res.success) {
                for (let file of res.data) {
                    if (file.fileName.length > 10) {
                        ...
                        $("#fileList ul").append(
                            `<li class="fl-li"><i class="fileId">${file.id}</i><img src="${getFileLogo(file.suffix)}"><label>${file.fileName}</label><span class="tooltiptext">${file.fileName}<span style="color: red"> - 双击下载</span></span></li>`
                        );
                    }
                }
            }
...
$(function () {
    ...
    $('#up-spread').click(function () {
        if ($('.up-content').css('display') == 'block') {
            ...
            $('#up-spread').attr('src', "./assets/bigFile/img/上.png");
        } else {
            $('.up-content').css('display', "block");
            $('#up-spread').attr('src', "./assets/bigFile/img/下.png");
        }
    });
    /**
     * 断点续传功能
